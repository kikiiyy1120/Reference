...
try{
	...
	conn_sql = DriverManager.getConnection(url_sql);
	...
	pstmt_sql = conn_ora.prepareStatement(sql);;
	date.setYear(120);
	date.setMonth(1);
	date.setDate(1);
	aim = "20200207";
	...
	while(!simpleDate.format(date).equals(aim)){
		...
		for(int i=0;i<data.size();i++){
			pstmt_sql.setString(i*14+1, data.get(i).sale_dt);
			pstmt_sql.setString(i*14+2, data.get(i).type);
			pstmt_sql.setString(i*14+3, data.get(i).day);
			pstmt_sql.setString(i*14+4, data.get(i).cmpr_dt);
			pstmt_sql.setString(i*14+5, data.get(i).gubun);
			pstmt_sql.setLong(i*14+6, data.get(i).goal_off);
			pstmt_sql.setLong(i*14+7, data.get(i).goal_on);
			pstmt_sql.setLong(i*14+8, data.get(i).goal_all);
			pstmt_sql.setLong(i*14+9, data.get(i).sale_off);
			pstmt_sql.setLong(i*14+10, data.get(i).sale_on);
			pstmt_sql.setLong(i*14+11, data.get(i).sale_all);
			pstmt_sql.setLong(i*14+12, data.get(i).bfsale_off);
			pstmt_sql.setLong(i*14+13, data.get(i).bfsale_on);
			pstmt_sql.setLong(i*14+14, data.get(i).bfsale_all);
		}
	...
	System.out.println("END_SQL:"+simpleDate.format(date));
