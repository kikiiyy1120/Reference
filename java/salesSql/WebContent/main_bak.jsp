...
if(session.getAttribute("id") == null){	
	...
	pstmt = conn.prepareStatement("INSERT INTO HB_CRM.VIEW_LOG VALUES(?, sysdate, ?)");
...
String cmpr_dt = "";
String sql = "";
sql = "SELECT CMPR_DT FROM DPS.PS_SCHEDULE WHERE SALE_DT = ?";
...
//관별일매출
sql = 
"WITH TMP AS (SELECT ? AS V_SALE_DT FROM DUAL)                                               		  "+ // 날짜 : YYYYMMDD -> 20180723
"SELECT DECODE(TEAM,'경영지원부','5상품권','마리오1관','11관','마리오2관','22관','마리오3관','33관', '온라인', '4온라인(미입점)',TEAM) AS GUBUN     "+ //
"        ,SUM(ACT_OFF)       AS GOAL_OFF                                                              "+ //
"        ,SUM(ACT_ON)        AS GOAL_ON                                                               "+ //
"        ,SUM(ACT_ALL)       AS GOAL_ALL                                                              "+ //
"        ,SUM(SALE_OFF)      AS SALE_OFF                                                              "+ //
"        ,SUM(SALE_ON)       AS SALE_ON                                                               "+ //
"        ,SUM(SALE_ALL)      AS SALE_ALL                                                              "+ //
"        ,SUM(BFSALE_OFF)    AS BFSALE_OFF                                                            "+ //
"        ,SUM(BFSALE_ON)     AS BFSALE_ON                                                             "+ //
"        ,SUM(BFSALE_ALL)    AS BFSALE_ALL                                                            "+ //
"FROM                                                                                                 "+ //
"    (SELECT DPS.FN_GET_DEPT_PUMBUN_CD(A.STR_CD,A.PUMBUN_CD)                      AS TEAM             "+ // 관
"            , CASE WHEN B.BRAND_CD = '10001'                                                         "+ //
"                THEN NVL(SUM(A.ORIGIN_NORM_SAMT),0)                                                  "+ // 실적
"              END AS ACT_OFF                                                                         "+ // 실적
"            , CASE WHEN B.BRAND_CD = '10002' OR B.BRAND_CD = '10003'                                 "+ //
"                THEN NVL(SUM(A.ORIGIN_NORM_SAMT),0)                                                  "+ // 실적
"              END AS ACT_ON                                                                          "+ // 실적  
"            , NVL(SUM(A.ORIGIN_NORM_SAMT),0) AS ACT_ALL                                              "+ //
"            ,0  AS SALE_OFF                                                                          "+ //
"            ,0  AS SALE_ON                                                                           "+ //
"            ,0  AS SALE_ALL                                                                          "+ //
"            ,0  AS BFSALE_OFF                                                                        "+ //
"            ,0  AS BFSALE_ON                                                                         "+ //
"            ,0  AS BFSALE_ALL                                                                        "+ //
"    FROM DPS.PS_ACTPUMBUN A,DPS.PC_PBNMST B, TMP C                                                   "+ //
"    WHERE A.EXE_DT = C.V_SALE_DT                                                                     "+ //
"    AND B.PUMBUN_CD = A.PUMBUN_CD                                                                    "+ //
"    GROUP BY A.STR_CD,A.PUMBUN_CD,B.BRAND_CD                                                         "+ //
"    UNION ALL                                                                                        "+ //
"    SELECT DPS.FN_GET_DEPT_PUMBUN_CD(A.STR_CD,A.PUMBUN_CD)                      AS TEAM              "+ // 관
"            ,0  AS ACT_OFF                                                                           "+ //
"            ,0  AS ACT_ON                                                                            "+ //
"            ,0  AS ACT_ALL                                                                           "+ //
"            , CASE WHEN B.BRAND_CD = '10001'                                                         "+ //
"                THEN NVL(SUM(A.TOT_SALE_AMT),0)                                                      "+ // 실적
"              END AS SALE_OFF                                                                        "+ // 실적
"            , CASE WHEN B.BRAND_CD = '10002' OR B.BRAND_CD = '10003'                                 "+ //
"                THEN NVL(SUM(A.TOT_SALE_AMT),0)                                                      "+ // 실적
"              END AS SALE_ON                                                                         "+ // 실적  
"            , NVL(SUM(A.TOT_SALE_AMT),0) AS SALE_ALL                                                 "+ //
"            ,0  AS BFSALE_OFF                                                                        "+ //
"            ,0  AS BGSALE_ON                                                                         "+ //
"            ,0  AS BFSALE_ALL                                                                        "+ //
"    FROM DPS.PS_DAYPBN A,DPS.PC_PBNMST B, TMP C                                                      "+ //
"    WHERE A.SALE_DT =  C.V_SALE_DT                                                                   "+ //
"    AND B.PUMBUN_CD = A.PUMBUN_CD                                                                    "+ //
"    GROUP BY A.STR_CD,A.PUMBUN_CD,B.BRAND_CD                                                         "+ //
"    UNION ALL                                                                                        "+ //
"    SELECT DPS.FN_GET_DEPT_PUMBUN_CD(A.STR_CD,A.PUMBUN_CD)                      AS TEAM              "+ // 관
...
"            , CASE WHEN B.BRAND_CD = '10001'                                                         "+ //
"                THEN NVL(SUM(A.TOT_SALE_AMT),0)                                                      "+ // 실적
"              END AS BFSALE_OFF                                                                      "+ // 실적
"            , CASE WHEN B.BRAND_CD = '10002' OR B.BRAND_CD = '10003'                                 "+ //
"                THEN NVL(SUM(A.TOT_SALE_AMT),0)                                                      "+ // 실적
"              END AS BGSALE_ON                                                                       "+ // 실적  
"            , NVL(SUM(A.TOT_SALE_AMT),0) AS BFSALE_ALL                                               "+ //
"    FROM DPS.PS_DAYPBN A,DPS.PC_PBNMST B, TMP C                                                      "+ //
"    WHERE A.SALE_DT = (SELECT CMPR_DT FROM DPS.PS_SCHEDULE WHERE SALE_DT = C.V_SALE_DT)              "+ //
"    AND B.PUMBUN_CD = A.PUMBUN_CD                                                                    "+ //
"    GROUP BY A.STR_CD,A.PUMBUN_CD,B.BRAND_CD                                                         "+ //
"    )                                                                                                "+ //
"GROUP BY TEAM                                                                                        "+ //
"ORDER BY SUBSTR(GUBUN, 1, 1)                                                                                              "  //
;
ArrayList<SaleStruct> strDaySum = new ArrayList<SaleStruct>();
long[] strDaySumSum = new long[9];
pstmt = conn.prepareStatement(sql);
pstmt.setString(1, date);
rs = pstmt.executeQuery();
for(int i=0;rs.next();i++){
	strDaySum.add(i, new SaleStruct(
...
//팀별일매출
 sql = 
"WITH TMP AS (SELECT ? AS V_SALE_DT FROM DUAL)                                                                                                                          "+
"SELECT DECODE(TEAM,'영업1팀','11팀','영업2팀','22팀','재무팀','5상품권','전산팀','5상품권', '온라인', '4온라인(미입점)', '식음팀', '33팀', '영업3팀', '33팀', TEAM) AS GUBUN   "+       
"        ,SUM(ACT_OFF)       AS GOAL_OFF                                                                                                                                "+
"        ,SUM(ACT_ON)        AS GOAL_ON                                                                                                                                 "+
"        ,SUM(ACT_ALL)       AS GOAL_ALL                                                                                                                                "+
"        ,SUM(SALE_OFF)      AS SALE_OFF                                                                                                                                "+
...
"        ,SUM(BFSALE_ALL)    AS BFSALE_ALL                                                                                                                              "+
"FROM                                                                                                                                                                   "+
"    (SELECT DPS.FN_GET_TEAM_PUMBUN_CD(A.STR_CD,A.PUMBUN_CD)                      AS TEAM                                                                               "+
"            , CASE WHEN B.BRAND_CD = '10001'                                                                                                                           "+
"                THEN NVL(SUM(A.ORIGIN_NORM_SAMT),0)                                                                                                                    "+
"              END AS ACT_OFF                                                                                                                                           "+
"            , CASE WHEN B.BRAND_CD = '10002' OR B.BRAND_CD = '10003'                                                                                                   "+
"                THEN NVL(SUM(A.ORIGIN_NORM_SAMT),0)                                                                                                                    "+
"              END AS ACT_ON                                                                                                                                            "+
...
"    GROUP BY A.STR_CD,A.PUMBUN_CD,B.BRAND_CD                                                                                                                           "+
"    )                                                                                                                                                                  "+
"GROUP BY DECODE(TEAM,'영업1팀','11팀','영업2팀','22팀','재무팀','5상품권','전산팀','5상품권', '온라인', '4온라인(미입점)', '식음팀', '33팀', '영업3팀', '33팀', TEAM)          "+
"ORDER BY GUBUN                                                                                                                                                         "
;
ArrayList<SaleStruct> teamDaySum = new ArrayList<SaleStruct>();
long[] teamDaySumSum = new long[9];
pstmt = conn.prepareStatement(sql);
pstmt.setString(1, date);
rs = pstmt.executeQuery();
for(int i=0;rs.next();i++){
	teamDaySum.add(i, new SaleStruct(
			rs.getString("GUBUN").substring(1, rs.getString("GUBUN").length()), rs.getLong("GOAL_OFF"), rs.getLong("GOAL_ON")
		   , rs.getLong("GOAL_ALL"), rs.getLong("SALE_OFF"), rs.getLong("SALE_ON")
		   , rs.getLong("SALE_ALL"), rs.getLong("BFSALE_OFF"), rs.getLong("BFSALE_ON")
		   , rs.getLong("BFSALE_ALL")
...
"            ,0  AS BFSALE_ALL                                                                      "+     //
"    FROM DPS.PS_ACTPUMBUN A,DPS.PC_PBNMST B, TMP C                                                 "+     //
"    WHERE A.EXE_DT BETWEEN SUBSTR(C.V_SALE_DT,1,6) || '01' AND  C.V_SALE_DT                        "+     //
"    AND B.PUMBUN_CD = A.PUMBUN_CD                                                                  "+     //
"    GROUP BY A.STR_CD,A.PUMBUN_CD,B.BRAND_CD                                                       "+     //
"    UNION ALL                                                                                      "+     //
"    SELECT DPS.FN_GET_DEPT_PUMBUN_CD(A.STR_CD,A.PUMBUN_CD)                      AS TEAM            "+     // 관
...
"            ,0  AS BFSALE_ALL                                                                      "+     //
"    FROM DPS.PS_DAYPBN A,DPS.PC_PBNMST B, TMP C                                                    "+     //
"    WHERE A.SALE_DT BETWEEN SUBSTR(C.V_SALE_DT,1,6) || '01' AND  C.V_SALE_DT                       "+     //
"    AND B.PUMBUN_CD = A.PUMBUN_CD                                                                  "+     //
"    GROUP BY A.STR_CD,A.PUMBUN_CD,B.BRAND_CD                                                       "+     //
"    UNION ALL                                                                                      "+     //
"    SELECT DPS.FN_GET_DEPT_PUMBUN_CD(A.STR_CD,A.PUMBUN_CD)                      AS TEAM            "+     // 관
...
"            , NVL(SUM(A.TOT_SALE_AMT),0) AS BFSALE_ALL                                             "+     //
"    FROM DPS.PS_DAYPBN A,DPS.PC_PBNMST B, TMP C                                                    "+     //
"    WHERE A.SALE_DT BETWEEN                                                                        "+     //
"      TO_CHAR(ADD_MONTHS(TO_DATE(SUBSTR(C.V_SALE_DT,1,6) || '01','YYYYMMDD'),-12),'YYYYMMDD')      "+     //
"      AND TO_CHAR(ADD_MONTHS(TO_DATE(C.V_SALE_DT,'YYYYMMDD'),-12),'YYYYMMDD')                      "+     //
"      AND B.PUMBUN_CD = A.PUMBUN_CD                                                                "+     //
"    GROUP BY A.STR_CD,A.PUMBUN_CD,B.BRAND_CD                                                       "+     //
"    )                                                                                              "+     //
"GROUP BY TEAM                                                                                      "+     //
"ORDER BY GUBUN                                                                                          "
;
ArrayList<SaleStruct> strMonthSum = new ArrayList<SaleStruct>();
long[] strMonthSumSum = new long[9];
pstmt = conn.prepareStatement(sql);
pstmt.setString(1, date);
rs = pstmt.executeQuery();
for(int i=0;rs.next();i++){
	strMonthSum.add(i, new SaleStruct(
			rs.getString("GUBUN").substring(1, rs.getString("GUBUN").length()), rs.getLong("GOAL_OFF"), rs.getLong("GOAL_ON")
		   , rs.getLong("GOAL_ALL"), rs.getLong("SALE_OFF"), rs.getLong("SALE_ON")
		   , rs.getLong("SALE_ALL"), rs.getLong("BFSALE_OFF"), rs.getLong("BFSALE_ON")
		   , rs.getLong("BFSALE_ALL")
...
"    FROM DPS.PS_DAYPBN A,DPS.PC_PBNMST B, TMP C                                                                                                                             "+
"    WHERE A.SALE_DT                                                                                                                                                         "+
"      BETWEEN TO_CHAR(ADD_MONTHS(TO_DATE(SUBSTR(C.V_SALE_DT,1,6) || '01','YYYYMMDD'),-12),'YYYYMMDD')                                                                       "+
"      AND TO_CHAR(ADD_MONTHS(TO_DATE(C.V_SALE_DT,'YYYYMMDD'),-12),'YYYYMMDD')                                                                                               "+
"      AND B.PUMBUN_CD = A.PUMBUN_CD                                                                                                                                         "+
"    GROUP BY A.STR_CD,A.PUMBUN_CD,B.BRAND_CD                                                                                                                                "+
"    )                                                                                                                                                                       "+
...
"ORDER BY GUBUN                                                                                                                                                              "
;
ArrayList<SaleStruct> teamMonthSum = new ArrayList<SaleStruct>();
long[] teamMonthSumSum = new long[9];
pstmt = conn.prepareStatement(sql);
pstmt.setString(1, date);
rs = pstmt.executeQuery();
for(int i=0;rs.next();i++){
	teamMonthSum.add(i, new SaleStruct(
			rs.getString("GUBUN").substring(1, rs.getString("GUBUN").length()), rs.getLong("GOAL_OFF"), rs.getLong("GOAL_ON")
		   , rs.getLong("GOAL_ALL"), rs.getLong("SALE_OFF"), rs.getLong("SALE_ON")
		   , rs.getLong("SALE_ALL"), rs.getLong("BFSALE_OFF"), rs.getLong("BFSALE_ON")
		   , rs.getLong("BFSALE_ALL")
